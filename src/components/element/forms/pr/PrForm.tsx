...
import { typeSelectorArr } from '@/data/prData'
...
const PrForms: FC<PrFormsProps> = ({ departmentData, modalSuccess, modalError, modalInfo, modalDownload, modalTgError, modalTGBotError }) => {
  ...
  const data = typeSelectorArr.find((type: any) => type.label === pr.type)
        ...
        <div>
          <div>
                ...
                {/*  */}
                  ...
                  {
                    typeSelector.createSelector(pr.type, (e: any) => {setPr({...pr, type: e.target.value})})
                  }
                              ...
                              switch (field.typeField) {
                                ...
                                case 'file':
                                  return new MyFileField(field.title, field.placeholder, field.name).uploadFile(
                                    pr[field.name],
                                    (e: any) => { setPr({ ...pr, [field.name]: e.target.files })}
                                  );
                                case 'area':
